<?php
    class Blogs {
        ...
        {
            $sql = "INSERT INTO Blogs(`blog_title`,`Blog_Slug`,`language`,`blog_image_name`,`blog_content`,`read_time`) VALUES(?,?,?,?,?,?)";
        ...
        public function updateBlogs()
        {
            if(empty($this -> Blog_Image) || $this -> Blog_Image == ""){
                $sql = "UPDATE Blogs SET blog_title= ?,`language` = ?,Blog_Slug= ?,blog_content = ?,read_time = ?,Last_Update = ? WHERE id =?";
                $query = $this -> conn -> prepare($sql);
                ...
                $sql = "UPDATE Blogs SET blog_title= ?,`language` = ?,Blog_Slug= ?,blog_image_name = ?,blog_content = ?,read_time = ? ,Last_Update = ? WHERE id =?";
                $query = $this -> conn -> prepare($sql);
